# -*- coding: utf-8 -*-
r"""
Pearl command line interface (CLI)
==============
Composed by 4 main commands:
    train       Used to train a machine translation metric.
    score       Uses Pearl to score a list of MT outputs.
...
from pearl.corpora import corpus2download, download_corpus
...
def score(model, source, hypothesis, reference, cuda, batch_size, to_json):
    ...
    click.secho(
        "Pearl system score: {:.3f}".format(sum(scores) / len(scores)), fg="yellow"
    )
