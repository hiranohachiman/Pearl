...
Schedulers
==============
    Leraning Rate schedulers used to train Pearl models.
"""
...
class ExponentialDecayWarmupPolicy:
    """Policy for updating the LR of the ExponentialDecayWarmup scheduler.
    With this class LambdaLR objects became picklable.
    """

    def __init__(self, warmup_steps, decay_rate=0.9998):
        self.warmup_steps = warmup_steps
    ...
    def __call__(self, current_step):
        if current_step < self.warmup_steps:
            return float(current_step) / float(max(1, self.warmup_steps))
        return self.decay_rate ** (current_step - self.warmup_steps)


class ExponentialDecayWarmup(LambdaLR):
    """
    Create a schedule with a learning rate that:
    1) Linearly increases during warmup period
    2) Exponentially decays after warmup (multiplied by decay_rate each step)

    :param optimizer: torch.optim.Optimizer
    :param warmup_steps: Linearly increases learning rate from 0 to 1*learning_rate over warmup_steps.
    :param decay_rate: Rate at which learning rate decays after warmup (default: 0.9998)
    :param last_epoch:
    """
    ...
    def __init__(
        ...
        super(ExponentialDecayWarmup, self).__init__(
            optimizer, ExponentialDecayWarmupPolicy(warmup_steps, decay_rate), last_epoch
        )
    ...
    def from_hparams(
        ...
        """Initializes a learning rate scheduler with warmup period and exponential decay."""
        return ExponentialDecayWarmup(optimizer, hparams.warmup_steps)
...
str2scheduler = {
    ...
    "exponential_warmup": ExponentialDecayWarmup,
}
