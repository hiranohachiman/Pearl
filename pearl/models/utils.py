...
def load_clip_features(feature_dir: str, img=True, mt=True, ref=True, num_splits: int = 5):
    """
    Function to load and merge split-saved features
    
    Args:
        feature_dir: Directory path where feature files are saved
        num_splits: Number of split files (default: 5)
        
    Returns:
        features_dict: Dictionary containing each feature
    """
    ...
    # Load and merge split files in order
    for i in range(num_splits):
        if img:
            img_part = dict(np.load(feature_dir / f"clip_img_features_{i+1}.npz", allow_pickle=True)['features'].item())
        if mt:
            mt_part = dict(np.load(feature_dir / f"clip_mt_features_{i+1}.npz", allow_pickle=True)['features'].item())
        if ref:
            ref_part = dict(np.load(feature_dir / f"clip_ref_features_{i+1}.npz", allow_pickle=True)['features'].item())
        # Update dictionaries
        if img:
            img_features.update(img_part)
        if mt:
            mt_features.update(mt_part)
        if ref:
            ref_features.update(ref_part)
