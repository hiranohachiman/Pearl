...
from pearl.metrics import RegressionReport
from pearl.models.model_base import ModelBase
from pearl.models.utils import average_pooling, max_pooling, move_to_cpu, move_to_cuda
...
def quantile_loss(y_true, y_pred):
    error = y_true - y_pred
    tau = 0.1
    loss = torch.maximum(tau * error, (tau - 1) * error) ** 2
    mse = (error ** 2).sum()
    return torch.sum(loss) + mse
...
class Estimator(ModelBase):
    ...
    def _build_loss(self):
        ...
        elif self.hparams.loss == "quantile":
    ...
    def compute_loss(
        ...
        return self.loss(model_out["ref_score"].view(-1), targets["score"]) + self.loss(model_out["img_score"].view(-1), targets["score"])
    ...
    def predict(
        ...
        with torch.no_grad():
            ...
            for model_input in model_inputs:
                ...
                model_img_score = model_out["img_score"].numpy().tolist()
                model_ref_score = model_out["ref_score"].numpy().tolist()
                for i in range(len(model_scores)):
                    scores.append(model_scores[i])
                    img_scores.append(model_img_score[i])
                    ref_scores.append(model_ref_score[i])
        ...
        for i in range(len(scores)):
            ...
            samples[i]["img_score"] = img_scores[i]
            samples[i]["ref_score"] = ref_scores[i]
        return samples, scores, img_scores, ref_scores
