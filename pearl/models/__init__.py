...
from .estimators import PearlEstimator, QualityEstimator
from .model_base import ModelBase
from .ranking import PearlRanker

str2model = {
    "PearlEstimator": PearlEstimator,
    "PearlRanker": PearlRanker,
...
def get_cache_folder():
    if "HOME" in os.environ:
        cache_directory = os.environ["HOME"] + "/.cache/torch/unbabel_pearl/"
...
def download_model(model: str, saving_directory: str = None) -> ModelBase:
    ...
    if os.path.isdir(saving_directory + model):
        ...
        raise Exception(f"{model} is not a valid Pearl model!")
...
def load_checkpoint(checkpoint: str) -> ModelBase:
    ...
    if os.path.exists(tags_csv_file):
        ...
        raise Exception(
            ...
            " Please clean your cache folder (~/.cache/torch/unbabel_pearl/) and try to download the model again."
        )
